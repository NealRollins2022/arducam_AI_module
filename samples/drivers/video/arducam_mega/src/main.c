...
{
    switch (buff[0]) {
    case 0x01: if (set_mega_resolution(buff[1]) == 0) take_picture_fmt = buff[1]; break;
    case 0x02: if (!preview_on) { set_mega_resolution(buff[1] | 0x10); video_stream_start(video); capture_flag = 1; } preview_on = 1; break;
    case 0x10: video_stream_start(video); take_picture(); video_stream_stop(video); break;
    case 0x21: if (preview_on) { video_stream_stop(video); set_mega_resolution(take_picture_fmt); } preview_on = 0; break;
    default: break;
    }
}
...
int main(void)
{
    ...
    while (1) {
        if (uart_available(recv_buffer)) recv_process(recv_buffer);
        video_preview();
        k_msleep(1);
    }
}
